			...
			uint64_t offset)
{
	...
	if (write(btree->fd, table, sizeof *table) != (ssize_t) sizeof *table) {
		fprintf(stderr, "btree: I/O error offset:%lf\n",offset);
		abort();
	}
...
static int btree_open(struct btree *btree, const char *idxname,const char *dbname)
{
...
static int btree_creat(struct btree *btree, const char *idxname,const char* dbname)
{
	memset(btree, 0, sizeof *btree);
...
int btree_init(struct btree *btree,const char* fname)
{
	...
	if(file_exists(idxname))
		...
		btree_creat(btree,idxname,dbname);
}
